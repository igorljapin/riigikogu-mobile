...
| `data/seating.json` | The one dataset only the desktop surface reads: each MP's session-hall seat position, keyed by uuid. Hand-maintained from the parliament's own plan at <https://www.riigikogu.ee/riigikogu/koosseis/saali-plaan/> — the API publishes no seat, that page does. Each entry carries the official `place` (1–120) so the file can be diffed against it; `row`/`col` are derived. See "Updating MP data" below. |
